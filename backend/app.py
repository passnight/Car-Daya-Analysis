...
# def Fuel():
#     myForm = request.form
#     model = myForm.to_dict()
#     handleFChart1Request(model)
...
def Power():
    ...
    global fuelSelectModel
    fuelSelectModel = dataForm["chooseModel"]
    print(json.dumps(fuelSelectModel))
    return json.dumps(fuelSelectModel)
...
# @app.route("/FChart1.json", methods=["GET", "POST"])
# @cross_origin(supports_credentials=True)
# def handleFChart1Request(model):
#     data = FuelDao.query(model)
#     return data
...
def handlePChart2Request():
    ...
    return json.dumps(data)
    global fuelSelectModel
    data = PowerDao.query(fuelSelectModel)
    print(data)
    return data
