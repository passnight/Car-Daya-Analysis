...
def getmodels():
    ...
    cursor1 = db.cursor()
    sql1 = "SELECT car_model FROM car_fuel_table"
...
def query(inputData):
    db = pymysql.connect(host="rm-bp1at82o2l9uonoizao.mysql.rds.aliyuncs.com", user="root",
                             password="Password123", database="car_big_data", charset="utf8")
    cursor = db.cursor()
    sql = "SELECT car_fuel_consumption_per_hundred_kilometers,car_comprehensive_fuel_consumption FROM car_fuel_table where car_model ='%s'"%(inputData)
    cursor.execute(sql)
...
def getType(inputData):
    db = pymysql.connect(host="rm-bp1at82o2l9uonoizao.mysql.rds.aliyuncs.com", user="root",
                             password="Password123", database="car_big_data", charset="utf8")
    cursor = db.cursor()
    sql = "SELECT car_energy_type FROM car_fuel_table where car_model ='%s'"%(inputData)
    cursor.execute(sql)
    results = cursor.fetchall()
    #print(type(results))  # 返回<class 'tuple'> tuple元组类型
    #print(results)
