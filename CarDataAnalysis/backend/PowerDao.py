...
def query(inputData):
    db = pymysql.connect(host="rm-bp1at82o2l9uonoizao.mysql.rds.aliyuncs.com", user="root",
                             password="Password123", database="car_big_data", charset="utf8")
    cursor = db.cursor()
    sql = "SELECT car_max_power,car_max_torque,car_max_horsepower,car_max_speed,car_acceleration FROM car_power_table where car_model ='%s'"%(inputData)
    cursor.execute(sql)
